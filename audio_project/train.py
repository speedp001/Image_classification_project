...
def train(model, train_loader, val_loader, epochs, DEVICE, optimizer, criterion) :
    ...
    for epoch in range(epochs) :
        ...
        if val_acc > best_val_acc :
            torch.save(model.state_dict(), './audio_project/audio_best.pt')
...
def main() :
    ...
    print(f"현재 사용 중인 디바이스: {device_mps}")
    ...
    #dataset
    train_dataset = CustomDataset("./audio_project/audio_data/train/", transform=train_transform)
    val_dataset = CustomDataset("./audio_project/audio_data/val/", transform=val_transform)


    #dataloader
    train_loader = DataLoader(train_dataset, batch_size=128, num_workers=4, pin_memory=True, shuffle=True)
    val_loader = DataLoader(val_dataset, batch_size=128, num_workers=4, pin_memory=True, shuffle=True)
