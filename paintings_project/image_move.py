...
# org_dir = "./paintings_project/paintings/"
# train_dir = "./paintings_project/data_art/train"
# val_dir = "./paintings_project/data_art/val"
...
#예외 데이터 처리
temp = "Expressionist"
org_dir = f"./paintings_project/paintings/{temp}"
...
for path in tqdm(file_path) :
    ...
    print(file_name)
    file_type = path.split("/")[3]
    file_type = file_type.split(".")[-1]
    if file_type != "jpg!d" :
        print(path)
        os.makedirs(f"./paintings_project/data_art/{temp}/", exist_ok=True)
        image = cv2.imread(path)
        cv2.imwrite(f"./paintings_project/data_art/{temp}/{file_name}.png", image)
        #저장경로와 입력시킬 image정보
